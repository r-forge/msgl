/*
 * msgl_multinomial_sparse.h
 *
 *  Created on: May 23, 2012
 *      Author: martin
 */

#ifndef MSGL_MULTINOMIAL_SPARSE_H_
#define MSGL_MULTINOMIAL_SPARSE_H_

extern "C" {

R::SEXP r_msgl_sparse_basic(R::SEXP r_x, R::SEXP r_classes, R::SEXP r_featureWeights, R::SEXP r_classWeights, R::SEXP r_alpha,
		R::SEXP r_lambda, R::SEXP r_needed_solutions, R::SEXP r_do_refit, R::SEXP r_config);

R::SEXP r_msgl_sparse_lambda_seq(R::SEXP r_x, R::SEXP r_classes, R::SEXP r_featureWeights, R::SEXP r_classWeights,
		R::SEXP r_alpha, R::SEXP r_numberOfModels, R::SEXP r_lambdaMin, R::SEXP r_config);

R::SEXP r_msgl_sparse_predict_classes(R::SEXP r_x, R::SEXP r_beta);

R::SEXP r_msgl_sparse_cv(R::SEXP r_x, R::SEXP r_classes, R::SEXP r_featureWeights, R::SEXP r_classWeights, R::SEXP r_alpha,
...
R::SEXP msgl_sparse_lambda_seq(R::SEXP r_x, R::SEXP r_classes, R::SEXP r_featureWeights, R::SEXP r_classWeights,
		R::SEXP r_alpha, R::SEXP r_numberOfModels, R::SEXP r_lambdaMin, R::SEXP r_config) {
	...
	if (config.verbose) {
		...
		msg << "Msgl lambda sequence - sparse data - starting";
		SGL_MSG(msg.str().c_str());
	}

	sgl::DimConfig dim_config = sgl::createDimConfig(featureWeights, classWeights);
	msgl::GroupedMatrixData<sgl::sparse_matrix> data(X, Y, true);

	msgl::multinomial_sparse obj_type(data);

	sgl::Interface<msgl::AlgorithmConfiguration, msgl::multinomial_sparse> sgl_optimizer(obj_type, dim_config, alpha, config);

	sgl::vector result = sgl_optimizer.lambda_sequence(sgl_optimizer.lambda_max(),
			get_value<sgl::numeric> (r_lambdaMin), get_value<sgl::natural> (r_numberOfModels));
...
R::SEXP r_msgl_sparse_lambda_seq(R::SEXP r_x, R::SEXP r_classes, R::SEXP r_featureWeights, R::SEXP r_classWeights,
	...
	try {
		...
		return msgl_sparse_lambda_seq(r_x, r_classes, r_featureWeights, r_classWeights, r_alpha, r_numberOfModels,
...
R::SEXP msgl_sparse_basic(R::SEXP r_x, R::SEXP r_classes, R::SEXP r_featureWeights, R::SEXP r_classWeights, R::SEXP r_alpha,
	...
	if (config.verbose) {
		std::ostringstream msg;
		msg << "Msgl basic - starting" << endl;
		msg << "Data matrix : " << X.n_rows << " x " << X.n_cols << " sparse matrix. Checksum (sum of all elements) = " << sum(X);
		SGL_MSG(msg.str().c_str());
	}
	...
	if (do_refit) {
		...
		//Build result R list
		res_ptr = auto_ptr<rList> (new rList(6));

		res_ptr.get()->attach(rObject(result_refit.get<0> ()), "beta.refit");
		res_ptr.get()->attach(rObject(result_refit.get<1> ()), "likelihood.refit");

	} else {

		//Build result R list
		res_ptr = auto_ptr<rList> (new rList(4));
	...
	res.attach(rObject(object_value), "likelihood");
	res.attach(rObject(function_value), "penalized");
...
R::SEXP msgl_sparse_predict_classes(R::SEXP r_x, R::SEXP r_beta) {
	...
	boost::tuple<sgl::matrix_field, sgl::matrix_field, sgl::natural_matrix> result = convert(
			predictor.predict(msgl::MatrixData<sgl::sparse_matrix>(X, true), as_block_vector(beta)));
...
R::SEXP r_msgl_sparse_predict_classes(R::SEXP r_x, R::SEXP r_beta) {
	...
	try {
		...
		return msgl_sparse_predict_classes(r_x, r_beta);
...
R::SEXP msgl_sparse_cv(R::SEXP r_x, R::SEXP r_classes, R::SEXP r_featureWeights, R::SEXP r_classWeights, R::SEXP r_alpha,
		R::SEXP r_lambda_seq, R::SEXP r_do_refit, R::SEXP r_fold, R::SEXP r_cv_indices, R::SEXP r_use_cv_indices,
	...
	if (config.verbose) {
		std::ostringstream msg;
		msg << "Msgl cv - sparse data - starting";
		SGL_MSG(msg.str().c_str());
	}
	...
	if (do_refit) {
		...
		boost::tuple<sgl::matrix_field, sgl::matrix_field, sgl::natural_matrix> result =
				convert(response_field.get<1> ());
...
R::SEXP r_msgl_sparse_cv(R::SEXP r_x, R::SEXP r_classes, R::SEXP r_featureWeights, R::SEXP r_classWeights, R::SEXP r_alpha,
	...
	try {
		...
		return msgl_sparse_cv(r_x, r_classes, r_featureWeights, r_classWeights, r_alpha, r_lambda_seq, r_do_refit, r_fold,
...
#endif /* MSGL_MULTINOMIAL_SPARSE_H_ */
