...
# Sparse group lasso 
fit2a <- msgl(x, classes, alpha = .5, lambda = lambda, standardize = FALSE)

# Lasso
fit3a <- msgl(x, classes, alpha = 1, lambda = lambda, standardize = FALSE)
...
# Sparse group lasso 
fit2b <- msgl(x, classes, alpha = .5, lambda = lambda, sparse.data = TRUE, standardize = FALSE)
if(max(abs(fit2a$beta[[100]]-fit2b$beta[[100]])) > 1e-10) stop()

# Lasso
fit3b <- msgl(x, classes, alpha = 1, lambda = lambda, sparse.data = TRUE, standardize = FALSE)
if(max(abs(fit2a$beta[[100]]-fit2b$beta[[100]])) > 1e-10) stop()
